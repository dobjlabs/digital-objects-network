...
| `txlib`                          | Transaction state machine: `StateHeader`, `GroundingWitness`, `Tx`, `TxBuilder` + `TxFinalized` rule. Party-blind: a mutation side says whether the builder can open it, never whose it is.           |
| `joint-tx`                       | Assembling one transaction across parties. Contributions a holder proves for someone else's assembly, the shared `TxPlan`, and the proving schedule (`graph`). Depends on `txlib`.                    |
| `interfaces/trade`               | `dobj-trade` demo CLI: a two-user swap over iroh driven by joint-tx. Reads objects/plugins from `$DOBJ_HOME`, proves in-process, posts via the relayer, imports the received object through dobjd.     |
...
| `just ensure-plugins`    | Installs `craft-basics.pexe` to `${DOBJ_HOME:-~/.dobj}/actions/` if none present.                                                                                                                                                                                                             |
...
State isolation between the two modes runs all the way through the driver: `dev-local` exports **`DOBJ_HOME`** (`libs/driver/src/paths.rs`), which replaces `~/.dobj` as the root holding objects, installed plugins, and `settings.json`. Anything resolving that root must honour it — `libs/pexe` carries its own copy of the resolution because it cannot depend on `driver`, and the `ensure-*` recipes read `${DOBJ_HOME:-$HOME/.dobj}`. The installed `dobj` CLI binary is the exception: it stays at `~/.dobj/bin` regardless, since that is an install location rather than driver state.
