...
mod tests {
    use super::{
        action_descriptors_by_name, class_names, class_ui_meta, visible_action_descriptors,
    };

    #[test]
    fn hides_dependency_only_actions_from_visible_catalog() {
        let visible = visible_action_descriptors();
        assert!(!visible.iter().any(|entry| entry.name == "UseWoodPick"));
        assert!(!visible.iter().any(|entry| entry.name == "UseStonePick"));
        assert!(visible
            .iter()
            .any(|entry| entry.name == "MineStoneWithWoodPick"));
    }

    #[test]
    fn derives_recursive_dependency_signatures() {
        let descriptors = action_descriptors_by_name();
        let mine_wood = descriptors.get("MineStoneWithWoodPick").unwrap();
        assert_eq!(mine_wood.input_classes, vec!["WoodPick"]);
        assert_eq!(mine_wood.output_classes, vec!["WoodPick", "Stone"]);

        let mine_stone = descriptors.get("MineStoneWithStonePick").unwrap();
        assert_eq!(mine_stone.input_classes, vec!["StonePick"]);
        assert_eq!(mine_stone.output_classes, vec!["StonePick", "Stone"]);
    }

    #[test]
    fn class_metadata_covers_all_derived_classes() {
        for class_name in class_names() {
            let meta = class_ui_meta(&class_name);
            assert!(
                !meta.emoji.is_empty(),
                "emoji missing for class {class_name}"
            );
            assert!(
                !meta.description.is_empty(),
                "description missing for class {class_name}"
            );
        }
    }
}
