---
name: bitcraft-craft-binder
description: Combine 3 sludge + 1 solvent into a binder.
---
...
4. Call `list_inventory`. Filter to live objects with `class_name == "Solvent"`. If fewer than 1, output exactly `no Solvent available — run craft-solvent` and stop.

5. Output candidates and prompt — exactly:

   ```
   1) <file_name of first live Solvent>
   2) <file_name of second live Solvent>
   ...
   pick Solvent:
   ```
...
6. Exit-word check (`cancel`/`quit`/`exit`/`q`/`nevermind` → `cancelled`). Otherwise parse as integer. Invalid → `invalid choice` and stop. Save the chosen path as `<solvent_path>`.

7. Call `run_action` with `action_id="CraftBinder"` and `input_object_paths=[...<sludge_paths>, <solvent_path>]` (flatten into a single list in the order shown).

8. On success, for each entry in the tool result's `outputs` array, output one line:

   `<class_name> → <output_path>`

   The class names you should see, in order: 1× Binder.
