---
name: bitcraft-craft-canvas
description: Craft Canvas (4 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftCanvas — Stitch 2 cloth + 1 fiber + 1 wax into canvas.
   2) CraftCanvasLye — Lye-treated canvas — frees tar from wax.
   3) CraftCanvasFlash — Flash canvas (PoW only) — gamble for throughput.
   4) CraftCanvasCrude — Crude canvas — recovers a cloth, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftCanvas"`, inputs: 2 Cloth, 1 Fiber, 1 Wax, outputs: 1 Canvas.
   - **2** → `action_id="CraftCanvasLye"`, inputs: 2 Cloth, 1 Fiber, 1 Lye, outputs: 1 Canvas.
   - **3** → `action_id="CraftCanvasFlash"`, inputs: 2 Cloth, 1 Fiber, 1 Wax, outputs: 1 Canvas.
   - **4** → `action_id="CraftCanvasCrude"`, inputs: 2 Cloth, 1 Fiber, 1 Wax, outputs: 1 Canvas, 1 Cloth.
