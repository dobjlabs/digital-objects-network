---
name: bitcraft-craft-casing
description: Craft Casing (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftCasing — Build a rocket casing from steel, canvas, bearings, coil, wire.
   2) CraftCasingCoated — Coated casing — uses coating to skip part of the VDF.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftCasing"`, inputs: 3 Steel, 2 Canvas, 2 Bearing, 1 Coil, 2 Wire, outputs: 1 Casing.
   - **2** → `action_id="CraftCasingCoated"`, inputs: 3 Steel, 2 Canvas, 2 Bearing, 1 Coil, 2 Wire, 1 Coating, outputs: 1 Casing.
