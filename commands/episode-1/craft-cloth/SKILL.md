---
name: bitcraft-craft-cloth
description: Craft Cloth (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftCloth — Weave 2 fiber into 1 cloth.
   2) CraftCanvasCrude — Crude canvas — recovers a cloth, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftCloth"`, inputs: 2 Fiber, outputs: 1 Cloth.
   - **2** → `action_id="CraftCanvasCrude"`, inputs: 2 Cloth, 1 Fiber, 1 Wax, outputs: 1 Canvas, 1 Cloth.
