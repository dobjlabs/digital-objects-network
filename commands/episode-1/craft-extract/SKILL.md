---
name: bitcraft-craft-extract
description: Craft Extract (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftExtract — Distill 2 acid into 1 extract + 1 sludge (PoW).
   2) CraftExtractCtrl — Controlled extract in the reaction chamber — extra sludge yield.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftExtract"`, inputs: 2 Acid, outputs: 1 Extract, 1 Sludge.
   - **2** → `action_id="CraftExtractCtrl"`, inputs: 2 Acid, outputs: 1 Extract, 2 Sludge.
