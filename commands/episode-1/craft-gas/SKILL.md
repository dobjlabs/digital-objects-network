---
name: bitcraft-craft-gas
description: Craft Gas (5 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   5) CraftRubberCrude — Crude rubber — recovers gas, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **5** → `action_id="CraftRubberCrude"`, inputs: 3 Gas, outputs: 1 Rubber, 1 Gas.
