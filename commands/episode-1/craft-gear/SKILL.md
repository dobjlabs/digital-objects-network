---
name: bitcraft-craft-gear
description: Craft Gear (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftGearCast — Cast 5 gears in the blast furnace using steel + mold.
   2) CraftGear — Mill 2 steel into 3 gears.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftGearCast"`, inputs: 1 Steel, 1 Mold, outputs: 5 Gear.
   - **2** → `action_id="CraftGear"`, inputs: 2 Steel, outputs: 3 Gear.
