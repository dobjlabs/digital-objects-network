---
name: bitcraft-craft-grease
description: Craft Grease (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftGrease — Distill tar into grease.
   2) CraftResinCrude — Crude resin — recovers grease, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftGrease"`, inputs: 2 Tar, outputs: 1 Grease.
   - **2** → `action_id="CraftResinCrude"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, outputs: 1 Resin, 1 Grease.
