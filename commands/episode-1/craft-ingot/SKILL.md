---
name: bitcraft-craft-ingot
description: Craft Ingot (3 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftIngot — Smelt one iron into one ingot.
   2) CraftIngotFlux — Smelt iron with flux for double-yield ingots.
   3) CraftIngotDrilled — Drilled-ingot variant — double yield, consumes drill-bit durability.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftIngot"`, inputs: 1 Iron, outputs: 1 Ingot.
   - **2** → `action_id="CraftIngotFlux"`, inputs: 1 Iron, 1 Flux, outputs: 2 Ingot.
   - **3** → `action_id="CraftIngotDrilled"`, inputs: 1 Iron, outputs: 2 Ingot.
