---
name: bitcraft-craft-reaction-chamber
description: Build a reaction chamber (requires MachineII).
---

# craft-reaction-chamber
...
7. Call `run_action` with `action_id="CraftReactionChamber"` and `input_object_paths=[...<circuit_paths>, ...<grease_paths>]` (flatten into a single list in the order shown).

8. On success, for each entry in the tool result's `outputs` array, output one line:

   `<class_name> → <output_path>`

   The class names you should see, in order: 1× ReactionChamber.
