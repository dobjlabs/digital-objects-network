---
name: bitcraft-craft-resin
description: Craft Resin (5 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftResin — Compound grease + solvent + rubber into resin (PoW + VDF).
   2) CraftResinPressurized — Pressurized resin — pressure-valve variant, deterministic.
   3) CraftResinStable — Stable resin — reaction-chamber + binder, double yield.
   4) CraftResinFlash — Flash resin (PoW only) — gamble for throughput.
   5) CraftResinCrude — Crude resin — recovers grease, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftResin"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, outputs: 1 Resin.
   - **2** → `action_id="CraftResinPressurized"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, outputs: 1 Resin.
   - **3** → `action_id="CraftResinStable"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, 1 Binder, outputs: 2 Resin.
   - **4** → `action_id="CraftResinFlash"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, outputs: 1 Resin.
   - **5** → `action_id="CraftResinCrude"`, inputs: 1 Grease, 1 Solvent, 1 Rubber, outputs: 1 Resin, 1 Grease.
