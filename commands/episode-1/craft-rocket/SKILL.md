---
name: bitcraft-craft-rocket
description: Craft Rocket (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftRocket — Final assembly: engine + casing + payload + 2 resin → 1 rocket.
   2) CraftRocketCat — Catalysed rocket — reaction-chamber + catalyst, deterministic.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftRocket"`, inputs: 1 Engine, 1 Casing, 1 Payload, 2 Resin, outputs: 1 Rocket.
   - **2** → `action_id="CraftRocketCat"`, inputs: 1 Engine, 1 Casing, 1 Payload, 2 Resin, 1 Catalyst, outputs: 1 Rocket.
