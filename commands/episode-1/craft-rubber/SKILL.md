---
name: bitcraft-craft-rubber
description: Craft Rubber (3 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftRubber — Vulcanize 3 gas into 1 rubber (PoW).
   2) CraftRubberFlash — Flash rubber (PoW only) — fast variant.
   3) CraftRubberCrude — Crude rubber — recovers gas, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftRubber"`, inputs: 3 Gas, outputs: 1 Rubber.
   - **2** → `action_id="CraftRubberFlash"`, inputs: 3 Gas, outputs: 1 Rubber.
   - **3** → `action_id="CraftRubberCrude"`, inputs: 3 Gas, outputs: 1 Rubber, 1 Gas.
