---
name: bitcraft-craft-solvent
description: Craft Solvent (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftSolvent — PoW solvent from fuel + acid.
   2) CraftSolventLye — VDF solvent variant — uses lye to free acid.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftSolvent"`, inputs: 1 Fuel, 1 Acid, outputs: 2 Solvent.
   - **2** → `action_id="CraftSolventLye"`, inputs: 2 Lye, 1 Fuel, 1 Water, outputs: 2 Solvent.
