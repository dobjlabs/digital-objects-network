---
name: bitcraft-craft-wire
description: Craft Wire (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) CraftWire — Draw 1 plate into 3 wire.
   2) CraftCircuitCrude — Crude circuit — recovers a wire, slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="CraftWire"`, inputs: 1 Plate, outputs: 3 Wire.
   - **2** → `action_id="CraftCircuitCrude"`, inputs: 2 Wire, 1 Steel, outputs: 1 Circuit, 1 Wire.
