---
name: bitcraft-farm-cane
description: Harvest cane with a short VDF.
---
...
1. Call `run_action` with `action_id="FarmCane"` and `input_object_paths=[]`.

2. On success, for each entry in the tool result's `outputs` array, output one line:

   `<class_name> → <output_path>`

   The class names you should see, in order: 1× Cane.
