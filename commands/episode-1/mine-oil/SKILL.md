---
name: bitcraft-mine-oil
description: Mine Oil (2 recipe variants).
---
...
1. Output exactly the following recipe menu, then end the turn and wait for the user's reply:
   ...
   ```
   1) MineOil — Drill oil by proving a PoW grind.
   2) CraftRefineryCrude — Crude refinery — recovers half the oil but slower.
   pick recipe:
   ```
...
3. Branch on the chosen recipe number:
   ...
   - **1** → `action_id="MineOil"`, inputs: no input objects, outputs: 1 Oil.
   - **2** → `action_id="CraftRefineryCrude"`, inputs: 2 Oil, 1 Water, outputs: 3 Tar, 1 Fuel, 1 Gas, 1 Oil.
