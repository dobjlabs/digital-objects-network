---
name: bitcraft-mine-sulfur
description: Surface sulfur deposits with a short VDF survey.
---
...
1. Call `run_action` with `action_id="MineSulfur"` and `input_object_paths=[]`.

2. On success, for each entry in the tool result's `outputs` array, output one line:

   `<class_name> → <output_path>`

   The class names you should see, in order: 1× Sulfur.
