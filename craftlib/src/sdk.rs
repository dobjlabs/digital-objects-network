...
mod tests {
    ...
    use pod2utils::set;
    use txlib::{StateRoot, Tx, rand_raw_value};
    use vdfpod::VdfPod;
    ...
    fn test_sdk() {
        ...
        fn use_pick_details(name: &'static str, vdf_iters: usize) -> Vec<Detail> {
            vec![
                Condition(
                ...
                Var(
                    ...
                    Box::new(|ctx| {
                        let obj = ctx.vars.get_dict(name).unwrap();
                        let mut durability =
                            i64::try_from(obj.get(&Key::from("durability")).unwrap().typed())
                                .unwrap();
                        durability -= 1;
