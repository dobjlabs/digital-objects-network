...
Each OR branch represents one valid way the object could have reached its current state. When proving `IsClassName(state)`, the prover supplies the private witness for whichever branch actually applies. The verifier learns only that _some_ valid branch holds — not which one.
