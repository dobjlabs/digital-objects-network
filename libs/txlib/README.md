...
2. **Replay.** Conceptually, replay walks the event tree and applies each step's state change and guard dispatch. In pseudocode:
   ...
   The replay spine threads a per-transaction **context** dict `{state_header, tx_commitment}` where `tx_commitment` is `tx_final`, the finalized transaction's commitment. Guards receive `context.state_header` (they are proven while the transaction is still being assembled, before `tx_final` exists), so guard statements are scoped to the grounding block only. Every `nullify` goes through `EndorseSpend`, which bundles the consumed object's nullifier derivation with a **spend endorsement**, `H(old.key, H(context, "txlib-endorsement-v1"))`. Both derivations require opening the consumed object's dict at `key`, so only the object's owner can produce the statement, and it cannot be copied from another transaction's proof because the context differs. They are bundled into one predicate because a custom predicate application is a scarce per-POD resource while the `Hash` clauses are nearly free, and because it makes `EndorseSpend` the single statement a party exports when contributing a spend to a jointly-assembled transaction. A spend is therefore only valid inside the exact transaction, and against the exact state header, that the owner endorsed -- which is what makes jointly-assembled transactions safe: nothing can finalize until every consumed object's owner has endorsed the final transaction commitment, and an endorsement expires with its state header.
...
4. **`TxFinalized`.** The public entry point. It seeds the chain (`chain_start = H(live_set, {})`), pins the initial `before_tx` schema (`nullifiers = {}`, `chain_start = chain_end = {}`, `live = inputs_set`) in a single `DictInsert` clause to remove malleability, and threads everything through one `ReplayActions` call. `TxFinalBindings` pins the context's two entries (the inner state header used for grounding, and `tx_commitment == tx_final`, which closes the endorsement loop for every spend in the replay) and surfaces the final `nullifiers` and `live` sets as public args (factored out so `TxFinalized` stays within the clause limit), letting the synchronizer fold them into its global nullifier and created sets. Public outputs: `(context, tx_final, nullifiers, live)`, where `context` is the two-entry dict commitment recomputed by the verifier from `(state_root, tx_final)` -- the recomputation is exact, so a context padded with extra keys can never be accepted.
