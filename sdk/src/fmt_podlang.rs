...
/// the bare name; intermediate timestamps suffix the index. Just SSA-style
...
/// A formatted handle to one position of a var, usable in `write!`. The
/// two variants encode the protocol/implementation split documented on
/// `fmt_chain_at` and `fmt_var_at` so the dispatch is at the type level
