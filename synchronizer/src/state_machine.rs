...
pub struct StateMachine {
    ...
    proof_parser: Arc<dyn ProofParser>,
}

impl StateMachine {
    pub fn new(db: Db, proof_parser: Arc<dyn ProofParser>) -> Result<Self> {
        let DerivedState {
            transactions,
            nullifiers,
            global_state_roots,
    ...
    pub fn process_blob(
        ...
        let Some(proof) = self.proof_parser.parse_blob(bytes)? else {
            info!(slot, block_number, "Blob did not contain a valid TxnFinalized proof; skipping");
            return Ok(());
        };

        // Validate that the proof's state_root_hash is in our known history.
        {
            let state = self
                .state
                .read()
                .map_err(|e| anyhow!("state read lock poisoned: {e}"))?;
            if !state.global_state_roots.contains(&proof.state_root_hash) {
                warn!(
                    slot,
                    block_number,
        ...
        // Apply state updates atomically: check uniqueness before writing anything.
        {
            ...
            if !state.transactions.insert(proof.tx_hash) {
            ...
            for nullifier in &proof.nullifiers {
                if state.nullifiers.contains(nullifier) {
                    ...
                    state.transactions.remove(&proof.tx_hash);
                    return Ok(());
                }
            }

            self.db.persist_transaction(proof.tx_hash, slot, block_number)?;
            for nullifier in &proof.nullifiers {
...
mod tests {
    use super::*;
    use crate::db::Db;
    use crate::proof::MockProofParser;
    use hex::ToHex;
    ...
    fn make_sm() -> (StateMachine, TempDir) {
        ...
        let sm = StateMachine::new(db, Arc::new(MockProofParser)).unwrap();
        (sm, dir)
    }
    ...
    fn txn_bytes(tx_hash: Hash, nullifiers: &[Hash], state_root: Hash) -> Vec<u8> {
    ...
    fn test_happy_path_single_tx() {
        ...
        sm.process_blob(&txn_bytes(tx_hash, &[nullifier], gsr0), 1, Some(1))
            .unwrap();
    ...
    fn test_sequence_across_blocks() {
        ...
        sm.process_blob(&txn_bytes(tx1, &[null1], gsr0), 1, Some(1))
            .unwrap();
        ...
        sm.process_blob(&txn_bytes(tx2, &[null2], gsr1), 2, Some(2))
            .unwrap();
    ...
    fn test_old_gsr_still_valid() {
        ...
        let tx1 = unique_hash(1);
        sm.process_blob(&txn_bytes(tx1, &[], gsr0), 1, Some(1))
            .unwrap();
        sm.advance_block(1).unwrap();

        // tx2 is grounded against gsr0, not the newer gsr1 — still valid
        let tx2 = unique_hash(2);
        sm.process_blob(&txn_bytes(tx2, &[], gsr0), 1, Some(1))
            .unwrap();
    ...
    fn test_duplicate_tx_rejected() {
        ...
        let tx_hash = unique_hash(1);
        let bytes = txn_bytes(tx_hash, &[], gsr0);
    ...
    fn test_duplicate_nullifier_rejected() {
        ...
        let tx1 = unique_hash(1);
        sm.process_blob(&txn_bytes(tx1, &[null], gsr0), 1, Some(1))
            .unwrap();

        let tx2 = unique_hash(2);
        sm.process_blob(&txn_bytes(tx2, &[null], gsr0), 1, Some(1))
    ...
    fn test_nullifier_collision_is_atomic() {
        ...
        let tx1 = unique_hash(1);
        sm.process_blob(&txn_bytes(tx1, &[spent], gsr0), 1, Some(1))
            .unwrap();

        // tx2 has [fresh_a, spent, fresh_b] — 'spent' is a duplicate
        let tx2 = unique_hash(2);
        sm.process_blob(&txn_bytes(tx2, &[fresh_a, spent, fresh_b], gsr0), 1, Some(1))
    ...
    fn test_unknown_gsr_rejected() {
        ...
        let tx_hash = unique_hash(1);
        sm.process_blob(&txn_bytes(tx_hash, &[], bogus_gsr), 1, Some(1))
            .unwrap();
